...
// 1) direct record: { id, helmetId, ... }
// 2) Fabric iterator style: { Key, Record: {...} }
function formatFabricRecordToRow(item) {
  const record = item?.Record ?? item;

  return {
    id: record.id ?? item?.Key,
    recordId: record.id ?? item?.Key,
    device: record.responderName,
    deviceId: record.helmetId,
    ...
    environmentalTemp: record.environmentalTemp,
...
export default function DeviceData() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const { data } = await api.get("/fabric/records");
      const records = (data.data || []).map(formatFabricRecordToRow);
      setAllData(records);
    } catch (err) {
      setDataError(err.response?.data?.message || "Failed to load Fabric sensor data");
      setAllData([]);
    } finally {
  ...
  const filtered = allData.filter((row) => {
    ...
    const matchSearch =
      ...
      (row.alert || "").toLowerCase().includes(q);
  ...
  const todayCount = allData.filter((r) => (r.timestamp || "").startsWith(todayIso)).length;
  ...
  return (
    <>
      <div className="flex flex-col gap-6">
        ...
        {/* All Received Fabric Data */}
        <div className="bg-white rounded-2xl border border-slate-200 p-8 flex flex-col gap-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h2 className="text-[15px] font-medium text-slate-800">All Received Fabric Data</h2>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-20 min-w-[200px] py-1">
                  {deviceOptions.map((opt) => (
                    <button
                      key={opt.value}
                      onClick={() => handleDeviceFilter(opt.value)}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by responder, device, timestamp, status, or alert…"
              value={search}
          ...
          {dataError && (
            <div className="bg-rose-50 border border-rose-200 text-rose-600 text-xs px-4 py-2.5 rounded-xl flex items-center justify-between">
              {dataError}
              <button onClick={fetchAllData} className="underline ml-2 shrink-0">
                Retry
              </button>
            </div>
          )}
          ...
          <div className="rounded-xl border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-5 bg-slate-50 px-5 py-2.5 text-[11px] uppercase tracking-wider text-slate-400 font-medium border-b border-slate-200">
              <span>Responder</span>
              <span>Helmet ID</span>
              <span>Timestamp</span>
              <span>Status</span>
            ...
            {loadingData ? (
              ...
              paginated.map((row, i) => (
                <div
                  key={row.id ?? i}
                  className="grid grid-cols-5 items-center px-5 py-3.5 border-t border-slate-100 text-sm text-slate-700 hover:bg-blue-50/40 transition"
                >
